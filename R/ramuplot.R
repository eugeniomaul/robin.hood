...
}else if(is.null(x.ticks) & (mod.func=="poisson" | mod.func=="nbreg")){
	xaxis.title <- bquote(paste("log Incidence Rate Ratio (95% CI) ", .(y.varname),sep="")) 
}else if(!is.null(x.ticks) & (mod.func=="poisson" | mod.func=="nbreg")){
	xaxis.title <- paste("Incidence Rate Ratio (95% CI) ", y.varname,sep="") 
...
if((mod.func=="logistic" | mod.func=="cox" | model.family=="binomial" | mod.func=="poisson" | mod.func=="nbreg") & !is.null(x.ticks)){
